...
int game_state2; // Player 2's state. 0 = main_menu, 1 = game, 2 = game_over
...
/**
 * Player 1's infinite update loop for the game. 
...
/**
 * Player 2's infinite update loop for the game. 
 * this thread runs concurrently with that other infinite loop, draw()
 * separating the draw loop from 'game-time'
 */
void update2(){
    while (game_state2 == STATE_GAME) {
		// sleep game tick (based on level)
		try{
			Thread.sleep(1000/p1.level);
		}
